...
const STATE_JUMP_ATTACK: String = "jump_attack"
...
func init(player_node: CharacterBody2D) -> void:
	...
	states = {
		...
		STATE_JUMP_ATTACK: _create_state(STATE_JUMP_ATTACK, jump_attack_start, jump_attack_update),
...
func _setup_transitions() -> void:
	var transitions = [
		...
		[ANYSTATE, states[STATE_JUMP_ATTACK], &"jump_attack"],
		[states[STATE_JUMP_ATTACK], states[STATE_JUMP], &"jump"],
...
func crouch_start() -> void:
	player.animated_sprite.play(player.ANIMATIONS.CROUCH, 1.0, false)
...
func jump_attack_start() -> void:
	player.animated_sprite.play(player.ANIMATIONS.JUMP_ATTACK)
	SoundManager.play_sound(Sound._attack, "SFX")
	player.attack_timer.start()
	emit_signal("attack_started")


func jump_attack_update(_delta: float) -> void:
	if player.attack_timer.is_stopped():
		emit_signal("attack_ended")
		if player.is_on_floor():
			dispatch(&"state_ended")
